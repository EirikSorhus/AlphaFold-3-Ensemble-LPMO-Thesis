...
#SBATCH --account=nn9999k              # ENDRE: Ditt prosjekt-ID
#SBATCH --time=02:00:00                # Maks kjøretid (2 timer)
#SBATCH --mem=4G                       # Minne per node
#SBATCH --cpus-per-task=1              # Antall CPU-kjerner
#SBATCH --output=logs/lpmo_%j.out      # Output-fil (%j = job ID)
#SBATCH --error=logs/lpmo_%j.err       # Error-fil
...
# 'data_raw/uniprot_LPMO_raw.fasta', kan du hoppe over dette steget:
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo ""
    echo "Output files:"
    echo "  - data_raw/uniprot_LPMO_raw.fasta"
    echo "  - data_raw/cazy_LPMO_raw.fasta"
    echo "  - data_raw/LPMO_all_raw.fasta  (merged)"
    echo ""
    echo "Next steps:"
