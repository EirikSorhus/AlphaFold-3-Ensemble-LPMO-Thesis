...
trap 'shopt -s nullglob; files=(${SCRATCH}/*/*/*_data.json); if (( ${#files[@]} )); then cp -f "${files[0]}" "${AF3_INPUT_DIR}/input_msa_test.json"; fi' EXIT
...
apptainer exec --cleanenv \
  ...
  /opt/af3-venv/bin/python /opt/alphafold3/run_alphafold.py \
    --json_path=/root/af_input/af3_test.json \
