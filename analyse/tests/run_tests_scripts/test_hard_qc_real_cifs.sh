#!/bin/bash
#SBATCH --job-name=run_tests_hard_qc_real_cifs
#SBATCH --account=nn1003k
#SBATCH --partition=small
#SBATCH --time=00:30:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=3
#SBATCH --output=/cluster/work/projects/nn1003k/eirik/Masteroppgave/analyse/tests/run_tests_scripts/logs/run_tests_hard_qc_real_cifs_%j.log

set -euo pipefail

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    echo "[ERROR] This script must be submitted with sbatch, not run directly on the login node."
    echo "[INFO] Example: sbatch tests/run_tests_scripts/test_hard_qc_real_cifs.sh"
    exit 1
fi
...
run_dir="$out_dir/hard_qc_real_cifs_${job_suffix}"
mkdir -p "$run_dir"
...
echo "[INFO] Running focused QC regression tests"
pytest \
    tests/test_posebusters_runner.py \
    tests/test_io_contracts.py::TestProtonationContract::test_cif_to_pdb_writes_posebusters_pdb \
    tests/test_io_contracts.py::TestProtonationContract::test_cif_to_pdb_report_has_required_fields \
    tests/test_io_contracts.py::TestProtonationContract::test_posebusters_pdb_strips_metals_but_keeps_nonmetal_conect \
    tests/test_qc_gates.py::TestPoseBustersGate \
    tests/test_qc_gates.py::TestPrivateerGate \
...
echo "[INFO] Hard QC run completed"
echo "[INFO] Results directory: $run_dir"
echo "[INFO] QC report: $run_dir/qc_report.json"
echo "[INFO] Run summary: $run_dir/hard_qc_real_cifs_summary.json"