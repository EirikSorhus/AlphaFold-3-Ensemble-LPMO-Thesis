#!/bin/bash
#SBATCH --job-name=run_tests_residue_contact_real_cifs
#SBATCH --account=nn1003k
#SBATCH --partition=small
#SBATCH --time=00:30:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=1
#SBATCH --output=/cluster/work/projects/nn1003k/eirik/Masteroppgave/analyse/tests/run_tests_scripts/logs/run_tests_residue_contact_real_cifs_%j.log

set -euo pipefail

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    echo "[ERROR] This script must be submitted with sbatch, not run directly on the login node."
    echo "[INFO] Example: sbatch tests/run_tests_scripts/test_residue_contact_real_cifs.sh"
    exit 1
fi
...
run_id="residue_contact_real_cifs_${job_suffix}"
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            cat <<'EOF'
Usage:
    sbatch tests/run_tests_scripts/test_residue_contact_real_cifs.sh [options] [cif ...]
...
echo "[INFO] Running focused residue-contact unit tests"
pytest tests/test_residue_contact_extraction.py tests/test_prolif_ifp.py -q

echo "[INFO] Running residue-contact real-case harness on ${#cif_paths[@]} CIFs"
/cluster/work/projects/nn1003k/eirik/conda/analyse_env/bin/python \
    tests/run_tests_scripts/run_residue_contact_real_cifs.py \
    --run-dir "$run_dir" \
    --run-id "$run_id" \
    "${cif_paths[@]}"

echo "[INFO] Residue-contact real-case run completed"
echo "[INFO] Results directory: $run_dir"
echo "[INFO] Summary: $run_dir/residue_contact_real_cifs_summary.json"
echo "[INFO] Residue-contact output: $run_dir/pose_residue_contact_table.tsv"