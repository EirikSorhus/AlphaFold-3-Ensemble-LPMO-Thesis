#!/bin/bash
#SBATCH --job-name=run_tests_posebusters_real_cifs
#SBATCH --account=nn1003k
#SBATCH --partition=small
#SBATCH --time=00:20:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=2
#SBATCH --output=/cluster/work/projects/nn1003k/eirik/Masteroppgave/analyse/tests/run_tests_scripts/logs/run_tests_posebusters_real_cifs_%j.log

set -euo pipefail

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    echo "[ERROR] This script must be submitted with sbatch, not run directly on the login node."
    echo "[INFO] Example: sbatch tests/run_tests_scripts/test_posebusters_real_cifs.sh"
    exit 1
fi
...
run_dir="$out_dir/posebusters_real_cifs_${job_suffix}"
mkdir -p "$run_dir"
...
echo "[INFO] Running focused PoseBusters regression test"
pytest tests/test_posebusters_runner.py -q

echo "[INFO] Running PoseBusters-only validation on ${#cif_paths[@]} CIFs"
/cluster/work/projects/nn1003k/eirik/conda/analyse_env/bin/python \
    tests/run_tests_scripts/run_posebusters_real_cifs.py \
    --run-dir "$run_dir" \
    --run-id "posebusters_real_cifs_${job_suffix}" \
    "${cif_paths[@]}"

echo "[INFO] PoseBusters-only run completed"
echo "[INFO] Results directory: $run_dir"
echo "[INFO] Run summary: $run_dir/posebusters_real_cifs_summary.json"